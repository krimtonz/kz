...
static void poly_writer_append(poly_writer_t *writer, Vtx (*vtx)[3], uint32_t color){
    ...
    if(writer->prev_color != color){
        // new surface
        gDPSetPrimColor(writer->p++, 0, 0, (color >> 24) & 0xFF,
...
static void draw_cylinder(Gfx **hit_view_p, Gfx **hit_view_d, int16_t radius, int16_t height, int16_t x, int16_t y, int16_t z){
    ...
    if(cylinder_p == NULL){
        ...
        cylinder_vtx[0] = (Vtx)gdSPDefVtxN(0,0,0,0,0,0,-127,0,0xFF);
        cylinder_vtx[1] = (Vtx)gdSPDefVtxN(0,128,0,0,0,0,127,0,0xFF);
        for(int i=0;i<12;i++){
            // x = cos(2(pi)i / 12)
            // z = -sin(2(pi)i / 12)
...
void kz_col_view(){
    ...
    if(kz.collision_view_status == COL_VIEW_REGENERATE){
        ...
        size_t static_col_size = 0x11 + 9 * static_col_cnt;
