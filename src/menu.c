...
static void menu_navigate(menu_t *menu, menu_nav_t nav) {
    ...
    if (menu->selected_item != NULL && menu_item_trigger_event(menu->selected_item, MENU_EVENT_NAVIGATE, (void*)nav)) {
        return;
    }
    ...
    for(menu_item_t *item = menu->items.first; item != NULL; item = list_next(item)) {
...
{
    return menu_x(item->owner) + 
            (item->x_cell * item->owner->cell_width) + 
            (item->x_cell * item->owner->x_padding) + 
            item->x_offset;
}

int menu_item_y(menu_item_t *item)
{
    return menu_y(item->owner) + 
            (item->y_cell * item->owner->cell_height) + 
            (item->y_cell * item->owner->y_padding) + 
            item->y_offset;
}
...
int menu_item_trigger_event(menu_item_t *item, menu_event_t event, void **event_data)
{
    ...
    for(event_handler_t *handler = item->owner->item_events.first; handler != NULL; handler = list_next(handler)) {
        if (handler->subscriber == item && 
            (handler->event & event) == event && 
            ((menu_item_event_cb_t)handler->callback)(handler, event, event_data))
        {
            return true;
        }
    }
...
int menu_trigger_event(menu_t *menu, menu_event_t event, void **event_data) 
{
    ...
    if (menu->child != NULL && menu_trigger_event(menu->child, event, event_data)) {
        return true;
    }
    ...
    for(event_handler_t *handler = menu->events.first;handler;handler = list_next(handler)) {
        if (handler->subscriber == menu && 
            (handler->event & event) == event && 
            ((menu_event_cb_t)handler->callback)(handler, event, event_data)) 
        {
            return true;
        }
    }
